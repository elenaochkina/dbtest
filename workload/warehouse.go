...
func (s *warehouseWorkload) Run(ctx context.Context, dsn string, tel *telemetry.Telemetry) (Result, error) {
	...
	if err := benchmark.DropOrdersTable(ctx, pool); err != nil {
		return nil, fmt.Errorf("drop orders: %w", err)
	}
	if err := benchmark.DropWarehouseTable(ctx, pool); err != nil {
		return nil, fmt.Errorf("drop warehouse: %w", err)
	}
	...
	if err := benchmark.SeedWarehouses(ctx, pool, seeder, s.cfg.Warehouses, tel); err != nil {
		return nil, fmt.Errorf("seed warehouses: %w", err)
	}

	before, err := validator.ComputeChecksum(ctx, pool, "warehouse", tel)
	if err != nil {
		return nil, fmt.Errorf("checksum before: %w", err)
	}

	if err := benchmark.RunOrderCycle(ctx, pool); err != nil {
		return nil, fmt.Errorf("order cycle: %w", err)
	}

	after, err := validator.ComputeChecksum(ctx, pool, "warehouse", tel)
	if err != nil {
		return nil, fmt.Errorf("checksum after: %w", err)
	}
	...
	if delta != -10 {
		return nil, fmt.Errorf("expected stock delta -10, got %d", delta)
	}
	return after, nil
}
