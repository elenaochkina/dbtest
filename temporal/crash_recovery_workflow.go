...
func CrashRecoveryWorkflow(ctx workflow.Context, cfg CrashRecoveryWorkflowConfig) (err error) {
	...
	if err = workflow.ExecuteActivity(ctx, work.RunWarehouse, WorkloadInput{
		Cluster: cluster,
		Config:  workloadCfg,
	}).Get(ctx, nil); err != nil {
		return err
	...
	if err = workflow.ExecuteActivity(ctx, dur.Fingerprint, FingerprintInput{
		Cluster: cluster,
